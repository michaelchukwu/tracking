...
function sendRequest_tracking() {
	...
	try {
		http.open('POST', '/tracking', true);
		http.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		http.onreadystatechange = handleResponse_tracking;
		http.send('subject=' + subject);
	} catch (e) {} finally {
		document.getElementById("submit").disabled = false;
		document.getElementById("submit").value = 'Find';
	}
}
...
function check_values_tracking() {
	var valid = '';
	...
	var body = '';
		...
		if ($j('#contactform #subject').val() != undefined)
			if ($j('#contactform #subject').val() == '') {
				...
				if (!hasClass)
					$j('#contactform #subject').parent().append('<label for="contactname" generated="true" class="error">Please provide tracking number!</label>');
...
function handleResponse_tracking() {
	try {
		if ((http.readyState == 4) && (http.status == 200)) {
			var response = JSON.parse(http.responseText);
			if(response['id'] == undefined){
				document.getElementById("trackingDetails").style.display = "";
				$j('#contactform #subject').parent().append('<label for="contactname" generated="true" class="error">Shipment does not exist</label>');
			}else{
				
				var txt = "<table border='0' style='border=none !important;'>"+
						"<tr>"+
							"<td> Shipment ID </td>"+
							"<td>" + response['id'] +"<td>"+
						"</tr>"+
						...
						"<tr>" +
							...
							"<td>" + response['from_location'] + "<td>" +
						"</tr>" +
						"<tr>" +
							"<td> Destination </td>" +
							"<td>" + response['to_location'] + "<td>" +
						"</tr>" +
						"<tr>" +
							"<td> <strong>Current Location</strong> </td>" +
							"<td> <strong>" + response['current_location'] + "</strong> <td>" +
						"</tr>" +
						"<tr>" +
					...
					" </table>";
				document.getElementById("trackingDetails").style.display = "";
				document.getElementById("trackingDetails").innerHTML = txt;
				console.log(txt);
			// document.getElementById("confirmation").innerHTML = response;
			// jQuery('#trackingDetails').show();
			}
